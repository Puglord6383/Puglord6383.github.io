...
export class Circuit {
  ...
  createGroup(id, opts = {}) {
    ...
    const scopeRef = (ref) =>
      typeof ref === "string" && !ref.includes(":") ? `${id}:${ref}` : ref;
    ...
    const groupAPI = {
      ...
      setTransform(nx, ny, s) {
        state.x = nx;
        state.y = ny;
        if (typeof s === "number") state.scale = s;
        applyTransform();
      },
      setPosition(nx, ny) {
        state.x = nx;
      ...
      addBit(localId, x, y) {
        return _addBit.call(owner, scopeId(localId), x, y, layers, items);
      },
      addAND(localId, x, y, s = 1) {
        return _addAND.call(owner, scopeId(localId), x, y, s, layers, items);
      },
      addOR(localId, x, y, s = 1) {
        return _addOR.call(owner, scopeId(localId), x, y, s, layers, items);
      },
      addNOT(localId, x, y, s = 1) {
        return _addNOT.call(owner, scopeId(localId), x, y, s, layers, items);
      },
      addDisplay(localId, x, y, w = 132, h = 36) {
        return _addDisplay.call(
          owner,
          scopeId(localId),
          x,
          y,
      ...
      addDangling(localId, name, x, y, r) {
        return _addPin.call(
          owner,
          scopeId(localId),
          name,
      ...
      addWire(fromRef, toRef, via) {
        return _addWire.call(
          owner,
          scopeRef(fromRef),
          scopeRef(toRef),
          via,
      ...
      pin(ref) {
        return _getPin.call(owner, scopeRef(ref));
      },
      label(text, x, y, opts) {
        return _addLabel.call(owner, text, x, y, opts, layers);
      },
      R: owner.R,
    ...
    if (frame) {
      owner.onRender(() => {
        try {
          ...
          /* getBBox can fail if not in DOM or display:none */
        }
      });
    }
  ...
  onRender(cb) {
    if (typeof cb === "function") this._renderCallbacks.push(cb);
  }
  ...
  render() {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      for (const comp of this.comps.values()) {
        switch (comp.kind) {
          case "AND": {
            const aP = this.pins.get(comp.pins.inA),
              bP = this.pins.get(comp.pins.inB);
            const o = this.pins.get(comp.pins.out);
            o.v = aP.v && bP.v ? 1 : 0;
            o.dr = !!(aP.dr || bP.dr);
            break;
          ...
          case "NOT": {
            const i = this.pins.get(comp.pins.in),
              o = this.pins.get(comp.pins.out);
            o.v = i.v ? 0 : 1;
            o.dr = !!i.dr;
            break;
          }
...
function _addWire(fromRef, toRef, via, layers, items) {
  ...
  const w = {
    from: typeof fromRef === "string" ? fromRef : fromRef.id,
    to: typeof toRef === "string" ? toRef : toRef.id,
    via,
    el,
  };
  this.wires.push(w);
...
function _addBit(id, x, y, layers, items) {
  ...
  const tx = this._make("text", { class: "bit-label", x: x + 32, y: y + 24 });
  tx.textContent = "0";
  const out = _addPin.call(
    this,
    id,
    "out",
    x + 64 + this.R.node,
  ...
  g.addEventListener("click", () => {
    comp.state.val ^= 1;
    tx.textContent = String(comp.state.val);
    this.render();
  });
...
function _addAND(id, x, y, s = 1, layers, items) {
  const w = 72 * s,
    h = 72 * s,
    r = 36 * s;
  const path = this._make("path", {
    class: "gate-shape",
    d: `M${x} ${y} H${x + 36 * s} A${r} ${r} 0 0 1 ${x + 36 * s} ${y + h} H${x} Z`,
  });
  layers.gGates.appendChild(path);
  const inA = _addPin.call(
    this,
    id,
    "inA",
    x,
...
function _addNOT(id, x, y, s = 1, layers, items) {
  const triW = 44 * s,
    h = 72 * s;
  const poly = this._make("polygon", {
    class: "gate-shape",
    points: `${x},${y + 12 * s} ${x},${y + h - 12 * s} ${x + triW},${y + h / 2}`,
    fill: "none",
  });
  layers.gGates.appendChild(poly);
  const inn = _addPin.call(
    this,
    id,
    "in",
    x,
...
function _addDisplay(id, x, y, w = 132, h = 36, layers, items) {
  ...
  const tx = this._make("text", {
    class: "bit-label",
    x: x + w / 2,
    y: y + h / 2,
  });
  tx.textContent = "0: Composite";
  const input = _addPin.call(
    this,
    id,
    "in",
    x,
