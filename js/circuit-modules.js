export function buildPrimeDemo(C) {
  ...
  (function () {
...

    {
      ...
      const iA = C.pin(X1_OR.pins.inA),
        iB = C.pin(X1_OR.pins.inB);
      C.addWire(X1_AND_TOP.pins.out, X1_OR.pins.inA, [
        { x: oT.x, y: iA.y },
        { x: iA.x - 6, y: iA.y },
      ]);
      C.addWire(X1_AND_BOT.pins.out, X1_OR.pins.inB, [
        { x: oB.x, y: iB.y },
        { x: iB.x - 6, y: iB.y },
      ]);
    }
  ...
  (function () {
    const h = 72 * S;
    const xNot = COL.AND2 + 120;
    const topRef = "AND2.out",
      botRef = "AND3.out";
    const pTop = C.pin(topRef),
      pBot = C.pin(botRef);
    ...
    C.addWire(topRef, TOP2, [
      { x: C.pin(topRef).x, y: yTopT },
      { x: xNot - 30, y: yTopT },
    ]);
    ...
    C.addWire(botRef, BOT2, [
      { x: C.pin(botRef).x, y: yBotT },
      { x: xNot - 60, y: yBotT },
    ]);
    C.addWire(BOT2, N2_BOT.pins.in, [{ x: xNot - 6, y: yBotT }]);

    const oTop = C.pin(N2_TOP.pins.out),
      oBot = C.pin(N2_BOT.pins.out);
    ...
    const iA = C.pin(X2_OR.pins.inA);
    const iB = C.pin(X2_OR.pins.inB);
    C.addWire(X2_AND_TOP.pins.out, X2_OR.pins.inA, [
      { x: oT.x, y: iA.y },
      { x: iA.x - 6, y: iA.y },
    ]);
    C.addWire(X2_AND_BOT.pins.out, X2_OR.pins.inB, [
      { x: oB.x, y: iB.y },
      { x: iB.x - 6, y: iB.y },
    ]);
  })();
  ...
  (function () {
    ...
    let pO1, pO2;
    try {
      pO1 = C.pin("X1_XOR_OR.out");
      pO2 = C.pin("X2_XOR_OR.out");
    } catch {
      return;
    }
    ...
    C.addWire("X2_XOR_OR.out", BOT12, [
      { x: pO2.x, y: yBotT },
      { x: xNot - 60, y: yBotT },
    ]);
    C.addWire(BOT12, N12_BOT.pins.in, [{ x: xNot - 6, y: yBotT }]);

    const oT = C.pin(N12_TOP.pins.out),
      oB = C.pin(N12_BOT.pins.out);
    ...
    const o1 = C.pin(X12_AND_TOP.pins.out),
      o2 = C.pin(X12_AND_BOT.pins.out);
    const mid = (o1.y + o2.y) / 2;
    const yOr = mid - 36 * S;
    const xOr = o1.x + 20;
    const X12_OR = C.addOR("X12_XOR_OR", xOr, yOr, S);
    const iA = C.pin(X12_OR.pins.inA),
      iB = C.pin(X12_OR.pins.inB);
    C.addWire(X12_AND_TOP.pins.out, X12_OR.pins.inA, [
      { x: o1.x, y: iA.y },
      { x: iA.x - 6, y: iA.y },
    ]);
    C.addWire(X12_AND_BOT.pins.out, X12_OR.pins.inB, [
      { x: o2.x, y: iB.y },
      { x: iB.x - 6, y: iB.y },
    ]);
  })();
  ...
  (function () {
    const h = 72 * S;
    const xNot = COL.AND2 + 120;
    const topRef = "AND4B.out",
      botRef = "AND5B.out";
    const pTop = C.pin(topRef),
      pBot = C.pin(botRef);

    let sep1 = 40;
    const mid3 = (pTop.y + pBot.y) / 2;
    const yTopT = mid3 - sep1 / 2;
    const yBotT = mid3 + sep1 / 2;

    const N3_TOP = C.addNOT("X3_NOT_TOP", xNot, yTopT - h / 2, S);
    const TOP3 = C.addJunction("X3_TOP_BEHIND", xNot - 30, yTopT);
    C.addWire(topRef, TOP3, [
      { x: C.pin(topRef).x, y: yTopT },
      { x: xNot - 30, y: yTopT },
    ]);
    ...
    C.addWire(botRef, BOT3, [
      { x: C.pin(botRef).x, y: yBotT },
      { x: xNot - 60, y: yBotT },
    ]);
    C.addWire(BOT3, N3_BOT.pins.in, [{ x: xNot - 6, y: yBotT }]);

    const oTop = C.pin(N3_TOP.pins.out),
      oBot = C.pin(N3_BOT.pins.out);
    ...
    const iA = C.pin(X3_OR.pins.inA),
      iB = C.pin(X3_OR.pins.inB);
    C.addWire(X3_AND_TOP.pins.out, X3_OR.pins.inA, [
      { x: oT.x, y: iA.y },
      { x: iA.x - 6, y: iA.y },
    ]);
    C.addWire(X3_AND_BOT.pins.out, X3_OR.pins.inB, [
      { x: oB.x, y: iB.y },
      { x: iB.x - 6, y: iB.y },
    ]);
  })();
  ...
  (function () {
    ...
    try {
      ...
      C.addWire("X12_XOR_OR.out", TOP23, [
        { x: pO4.x, y: yMid },
        { x: xTop, y: yMid },
        { x: xTop, y: yTop },
      ]);
      ...
      /* if missing, leave top path un-driven */
    }

    const oT = C.pin(N23_TOP.pins.out),
      oB = C.pin(N23_BOT.pins.out);
    ...
    C.addWire(X23_AND_TOP.pins.out, X23_OR.pins.inA, [
      { x: o1.x, y: iA.y },
      { x: iA.x - 6, y: iA.y },
    ]);
    C.addWire(X23_AND_BOT.pins.out, X23_OR.pins.inB, [
      { x: o2.x, y: iB.y },
      { x: iB.x - 6, y: iB.y },
    ]);
  })();
  ...
  (function () {
    let pOut;
    try {
      pOut = C.pin("X23_XOR_OR.out");
    } catch {
      return;
    }
    const DISP = C.addDisplay(
      "PRIME_DISPLAY",
      pOut.x + 20,
      pOut.y - 18,
      132,
